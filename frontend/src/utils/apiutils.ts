...
export async function handlePost<T>(call: () => Promise<T>) {
   ...
   try {
      ...
      if (typeof err === "object" && err !== null) {
         ...
         if (message === undefined) message = err as GlobalErr;
         if (message === undefined) throw Error("Expected SpecificErr or GlobalErr.");
      }
   }
   return message;
}
   ...
   if (global.detail !== undefined || global.error !== undefined || global.message !== undefined) {
      ...
      const dfs = (obj: Record<string, any>) => {
         for (const [key, val] of Object.entries(obj)) {
            if (typeof val === "object" && val !== null && !Array.isArray(val)) dfs(val);
            else out[key] = val;
         }
      };
...
export function paymentStatusToPL(status: string) {
   return status == "pending" ? "Oczekujące" : status == "paid" ? "Opłacone" : "Darowane";
}
