...
export default function GroupAdd() {
    ...
    const load = async () => {
        ...
        const t3 = id ? getClassGroup.do(id) : Promise.resolve(undefined);
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        toast.promise(handlePost2(() => id ? editClassGroup(id, group) : createClassGroup(group)), {
            loading: 'Ładowanie...',
            success: () => {
                    ...
                    {(results: Results) => {
                        ...
                        return (
                            <form onSubmit={handleSubmit} noValidate className="space-y-3">
                                ...
                                <SelectWithLabel name="weekday" label="Dzień tygodnia" prompt="Wybierz" defaultValue={dft?.weekday ? getWeekday(dft.weekday) : undefined} options={weekdays} kind="select-classic" />
                                ...
                                <InputWithLabel name="start_date" type="date" label="Dzień rozpoczęcia" defaultValue={dft?.start_date ? dateToISOday(dft.start_date) : undefined} error={errors["start_date"]} kind="input-classic" />
                                <InputWithLabel name="end_date" type="date" label="Dzień zakończenia" defaultValue={dft?.end_date ? dateToISOday(dft.end_date) : undefined} error={errors["end_date"]} kind="input-classic" />
